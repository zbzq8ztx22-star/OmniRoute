...
export const SETUP_MODULES = Object.freeze(
  Object.fromEntries(
    Object.entries(CLI_TARGET_MANIFEST)
      .filter(([, entry]) => entry.configureRecipe?.mode === "module")
      .map(([id, entry]) => [
        id,
        Object.freeze({
          module: entry.configureRecipe.module,
          exportName: entry.configureRecipe.exportName,
        }),
      ])
  )
);
...
export function resolveConfigureTargetOptions(opts = {}) {
  ...
  if (!explicitRemote) {
    ...
    resolved.baseUrl = resolved.remote;
...
function normalizedCodexBaseUrl(opts) {
  let base = String(opts.baseUrl || opts.remote || `http://localhost:${opts.port || "20128"}`);
  base = base.replace(/\/+$/, "");
...
export function buildCodexProfile(modelId, ctx, baseUrl) {
  const lines = [
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  if (existsSync(filePath)) {
    await writePrivateFileAtomic(`${filePath}.bak`, readFileSync(filePath, "utf8"));
  }
  await writePrivateFileAtomic(
    filePath,
    buildCodexProfile(modelId, ctxWindow, normalizedCodexBaseUrl(opts))
  );
...
export async function runConfigureCommand(cli, opts = {}, cmd) {
  ...
  let result;
  if (target === "codex") {
    result = await configureCodex(chosenId, ctxWindow, { ...requestOpts, ...opts });
  } else {
