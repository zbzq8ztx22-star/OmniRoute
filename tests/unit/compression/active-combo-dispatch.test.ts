...
describe("active named combo resolution (Phase 2)", () => {
  it("activeComboId + combo present => that combo's stacked pipeline (regardless of enginesExplicit)", () => {
    ...
    assert.deepEqual(optedIn.stackedPipeline, combos.c1);
  });
  it("activeComboId null => falls through to derived default (not the combo)", () => {
    ...
    assert.equal(
      selectCompressionStrategy(config, null, 0, undefined, undefined, combos),
      "stacked"
    );
    assert.equal(
      selectCompressionStrategy(config, null, 0, undefined, undefined, combos, "allow-lossy"),
      "rtk"
    );
  });
